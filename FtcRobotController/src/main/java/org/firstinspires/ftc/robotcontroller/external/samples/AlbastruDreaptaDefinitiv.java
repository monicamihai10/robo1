...
@Autonomous(name="ADD", group="Pushbot")
public class AlbastruDreaptaDefinitiv extends LinearOpMode {
    ...
    public void runOpMode() {
       ...
       // range_sensorf = new RangeSensorWrapper("fata", hardwareMap, a2, telemetry);
        ...
        encoderDriveculoare(1, 3000, 1000, 1000, 3000);
        sleep(500);
    ...
    public void DriveBack(double speed, int counts) {
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() &&
                ...
                if(counts == 1520){
                    if (range_sensorf.getDistanceInCm() > 135) {
                        robot.roatast.setPower(0);
                        robot.roatadr.setPower(0);
                    }
                } else if (counts == 1250){
                    if (range_sensorf.getDistanceInCm() > 120) {            //CHECK THE DESIRED POSITION WITH THE RANGE SENSORS
                        robot.roatast.setPower(0);
                        robot.roatadr.setPower(0);
                    }
                } else if (counts == 950) {
                    if (range_sensorf.getDistanceInCm() > 115) {
                        robot.roatast.setPower(0);
                        robot.roatadr.setPower(0);
                    }
                }
    ...
    public void Rotire90(double speed, int counts){
        if (!opModeIsActive()) return;
    ...
    public void encoderDriveculoare(double speed, int counts, int counts2, int counts3, int counts4) {
        if (!opModeIsActive()) return;
        int Target1 = 0;
        ...
        if (opModeIsActive()) {
            ...
            robot.mingiServo.setPosition(0.20);
            ...
            if (color_sensor1.isBlue() || color_sensor3.isRed()) {
                telemetry.addData("ROSU", "Vad rosu daram rosu");
                telemetry.update();
                if (!opModeIsActive()) return;
                ...
                telemetry.addData("ALBASTRU ", "Vad albastru daram rosu");
                telemetry.update();
                if (!opModeIsActive()) return;
